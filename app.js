...
/* ==========================================================================
   ASSIGNMENT 5 — JavaScript (Req 13: Form Validation, Req 31: Progress Bars,
   Req 7: Dynamic Statistics already in DOMContentLoaded above)
   ========================================================================== */

/* --------------------------------------------------------------------------
   Req 31: Animated Progress Bars
   Triggered when the section scrolls into view (IntersectionObserver).
   -------------------------------------------------------------------------- */
(function initA5ProgressBars() {
    const bars = [
        { barId: 'a5bar1', pctId: 'a5pct1', target: 72 },
        { barId: 'a5bar2', pctId: 'a5pct2', target: 91 },
        { barId: 'a5bar3', pctId: 'a5pct3', target: 58 }
    ];

    function animateBars() {
        bars.forEach(({ barId, pctId, target }) => {
    ...
    // Trigger once when the CSS Lab section enters the viewport
    const labSection = document.getElementById('a5-csslab');
...
/* --------------------------------------------------------------------------
   Req 13: Reservation Form Validation
   Validates: Name (≥3 chars), Email (regex), Phone (###-###-####),
              Platform (select), Date (≥today), Time, Team Members (1–100)
   -------------------------------------------------------------------------- */
(function initA5ReservationForm() {
    const form = document.getElementById('a5ReservationForm');
    if (!form) return;

    // Set today as the min date
    const dateInput = document.getElementById('a5resDate');
    if (dateInput) {
    ...
    const fields = {
        ...
        a5resPhone: {
            validate: v => /^\d{3}-\d{3}-\d{4}$/.test(v.trim()),
        },
        a5resSeat: {
            validate: v => v !== '',
        },
    ...
    function validateField(id) {
        ...
        if (!el) return true;
        const group = el.closest('.a5-form-group');
        const ok = fields[id].validate(el.value);
        if (group) {
    ...
    Object.keys(fields).forEach(id => {
        ...
        if (el) {
            ...
            el.addEventListener('input',  () => {
                // Remove error immediately if field becomes valid while typing
                if (fields[id].validate(el.value)) {
                    const group = el.closest('.a5-form-group');
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!allValid) {
            ...
            const firstInvalid = form.querySelector('.a5-form-group.invalid input, .a5-form-group.invalid select');
            if (firstInvalid) firstInvalid.focus();
            return;
        }

        // Success — show SubFlow toast
        const name    = document.getElementById('a5resName').value.trim();
        const date    = document.getElementById('a5resDate').value;
        const time    = document.getElementById('a5resTime').value;
        const platform = document.getElementById('a5resSeat').value;

        showToast('success',
            'Consultation Booked!',
            `${name}, your ${platform} call is confirmed for ${date} at ${time}.`
        );

        form.reset();
        // Clear all invalid classes after reset
        form.querySelectorAll('.a5-form-group').forEach(g => g.classList.remove('invalid'));
    });

    // Expose toast helper (reuse the global one defined in the main DOMContentLoaded)
